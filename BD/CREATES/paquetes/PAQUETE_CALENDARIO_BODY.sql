    ...
    BEGIN
      ...
      IF COMPROBADOR = 0 THEN 
        ...
        FOR EQUIPO1 IN (SELECT ID FROM EQUIPOS) LOOP
            FOR EQUIPO2 IN (SELECT ID FROM EQUIPOS WHERE ID != EQUIPO1.ID) LOOP
                SELECT COUNT(*) INTO COMPROBADOR FROM partidos
                    ...
                    OR (ID_EQUIPO1 = EQUIPO2.ID AND ID_EQUIPO2 = EQUIPO1.ID);
                IF (COMPROBADOR = 0) THEN
                    INSERT INTO partidos (id, ID_EQUIPO1, ID_EQUIPO2)VALUES (ID_PARTIDOS, EQUIPO1.ID, EQUIPO2.ID);
        ...
        WHILE CONT_JORNADA <= 11 LOOP 
            ...
            CONT_PARTIDO := 0; 
            FOR PAREJA IN (SELECT ID_EQUIPO1, ID_EQUIPO2 FROM partidos ORDER BY DBMS_RANDOM.VALUE) LOOP
                SELECT COUNT(*) INTO COMPROBADOR FROM PARTIDOS, JORNADAS
                    WHERE (((   ID_EQUIPO1 = PAREJA.ID_EQUIPO1) 
                            OR (ID_EQUIPO2 = PAREJA.ID_EQUIPO1)
                            OR (ID_EQUIPO1 = PAREJA.ID_EQUIPO2) 
                            OR (ID_EQUIPO2 = PAREJA.ID_EQUIPO2))
                                AND ID_JORNADA = JORNADA_ID)
                        OR    
                          (((  ID_EQUIPO1 = PAREJA.ID_EQUIPO2 AND ID_EQUIPO2 = PAREJA.ID_EQUIPO1) 
                           OR (ID_EQUIPO1 = PAREJA.ID_EQUIPO1 AND ID_EQUIPO2 = PAREJA.ID_EQUIPO2)) AND ID_JORNADA IS NOT NULL);
                IF COMPROBADOR = 0 THEN
                    UPDATE PARTIDOS SET FECHA = FECHA_PARTIDO, LUGAR = 'CUPRA ARENA', ID_JORNADA = JORNADA_ID
                    WHERE (ID_EQUIPO1 = PAREJA.ID_EQUIPO1 AND ID_EQUIPO2 = PAREJA.ID_EQUIPO2);
            ...
            IF CONT_PARTIDO = 6 THEN
                ...
                END IF;
            ELSE 
                UPDATE PARTIDOS SET ID_JORNADA = NULL 
