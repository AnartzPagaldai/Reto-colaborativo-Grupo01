...
BEGIN
    ...
    FOR i IN 1..v_num_equipos LOOP
        SELECT COUNT(*) INTO v_num_personal FROM CONTRATOS_PERSONAL CP, PERSONALES P WHERE (CP.ID_EQUIPO = i AND P.OFICIO='ENTRENADOR') AND cp.id_personal = P.ID;
        IF v_num_personal < v_min_personal THEN
            RAISE_APPLICATION_ERROR(-20001, 'El equipo ' || i || ' no tiene entrenador (' || v_min_personal || ')');
        END IF;
    END LOOP;
...
INSERT INTO SPLIT (ANIO, TIPO, ESTADO) VALUES (TO_DATE('03/10/2023', 'DD/MM/YYYY'), 'VERANO', 'ABIERTO')
Error en la línea de comandos : 1 Columna : 13
Informe de error -
Error SQL: ORA-20001: El equipo 1 no tiene entrenador (1)
